  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 429 && attempt < maxRetries) {
            ...
            console.log(`[SpotScanner] CoinGecko rate limited, waiting ${waitTime}ms (retry ${attempt + 1}/${maxRetries})`);
        ...
        if ((fetchError.name === 'AbortError' || fetchError.cause?.code === 'UND_ERR_CONNECT_TIMEOUT') && attempt < maxRetries) {
          const waitTime = Math.pow(2, attempt) * 2000;
          console.log(`[SpotScanner] CoinGecko timeout, retrying in ${waitTime}ms (attempt ${attempt + 1}/${maxRetries})`);
          await new Promise(resolve => setTimeout(resolve, waitTime));
          continue;
        }
      ...
      if (attempt === maxRetries) {
        console.error(`[SpotScanner] Failed to fetch historical OHLCV for ${tokenAddress} after ${maxRetries} retries:`, error.message);
        return null;
      }
    }
  }
...
async function fetchCurrentMarketData(tokenAddress: string): Promise<{
  ...
  try {
    ...
    if (liquidity < 1000 && liquidity > 0) {
      console.log(`[SpotScanner] ⚠️  Low liquidity detected: $${liquidity.toFixed(2)} - may be unit conversion issue`);
      console.log(`[SpotScanner]     Raw value: ${bestPair.liquidity?.usd}`);
    }
    ...
    console.error(`[SpotScanner] Failed to fetch current data for ${tokenAddress}:`, error);
    return null;
  }
}
...
async function fetchSOLPrices(): Promise<{ prices: number[]; currentPrice: number }> {
  ...
  try {
    ...
    console.error('[SpotScanner] Failed to fetch SOL prices:', error);
    throw error;
  }
}
...
async function fetchOnChainData(tokenAddress: string): Promise<{
  ...
  try {
    ...
    if (error.message && (
      ...
      console.log(`[SpotScanner] ✅ Highly distributed token detected (too many holders to count)`);
      return {
    ...
    console.error(`[SpotScanner] Failed to fetch on-chain data:`, error);
    throw error;
  }
}
...
function getFallbackTokens(): Array<{ symbol: string; address: string }> {
  console.log('[SpotScanner] 🔄 Using fallback core tokens...');
  const coreTokens = [
  ...
  console.log(`[SpotScanner] Fallback tokens: ${coreTokens.map(t => t.symbol).join(', ')}`);
  return coreTokens;
}
...
async function discoverCandidateTokens(): Promise<Array<{ symbol: string; address: string }>> {
  try {
    ...
    if (!apiKey) {
      console.log('[SpotScanner] No Birdeye API key found - using fallback tokens');
    ...
    while (retries <= maxRetries) {
      ...
      if (response.status === 429 && retries < maxRetries) {
        ...
        console.log(`[SpotScanner] Birdeye rate limited, waiting ${waitTime}ms (retry ${retries + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, waitTime));
        retries++;
      } else {
        console.log(`[SpotScanner] Birdeye API error: ${response.status} - using fallback tokens`);
    ...
    if (!response!.ok) {
      console.log(`[SpotScanner] Birdeye API failed after ${maxRetries} retries - using fallback tokens`);
      return getFallbackTokens();
    }
    ...
    if (!data.success || !data.data?.items || data.data.items.length === 0) {
      console.log('[SpotScanner] No tokens found from Birdeye');
      return [];
    }
    ...
    console.log(`[SpotScanner] ✅ Discovered ${top10.length} candidate tokens from Birdeye`);
    if (tradingMode.mode === 'NORMAL') {
      console.log(`[SpotScanner]    Filtered out: ${filteredStablecoins} stablecoins, ${filteredLSTs} LSTs, ${filteredMemecoins} memecoins (NORMAL mode)`);
    } else {
      console.log(`[SpotScanner]    Filtered out: ${filteredStablecoins} stablecoins, ${filteredLSTs} LSTs (memecoins ALLOWED in AGGRESSIVE mode)`);
    }
    console.log(`[SpotScanner]    From ${data.data.items.length} total tokens (min $1M liquidity, $500K volume)`);
    if (top10.length > 0) {
      console.log(`[SpotScanner] Top candidates: ${top10.map(t => `${t.symbol} ($${(t.liquidity / 1e6).toFixed(1)}M liq)`).join(', ')}`);
    }

    return top10.map(({ symbol, address }) => ({ symbol, address }));
  } catch (error) {
    console.error('[SpotScanner] Failed to discover tokens from Birdeye:', error);
    return getFallbackTokens();
  }
}
...
export async function fetchSpotTokens(): Promise<any[]> {
  try {
    const tradingMode = getTradingMode();
    console.log(`[SpotScanner] Discovering tokens from Solana network... (Mode: ${tradingMode.mode})`);
    ...
    if (tradingMode.enablePumpFun) {
      try {
        console.log('[SpotScanner] 🚀 AGGRESSIVE MODE: Fetching Pump.fun memecoins...');
        ...
        console.log(`[SpotScanner] ✅ Pump.fun: ${filteredPumpfun.length} tokens passed filters`);
        ...
        console.log(`[SpotScanner] Total candidates: ${candidates.length} (${pumpfunCandidates.length} from Pump.fun)`);
      } catch (error: any) {
        console.log(`[SpotScanner] ⚠️ Failed to fetch Pump.fun tokens: ${error.message}`);
      }
    }
    ...
    console.log(`[SpotScanner] ✅ Fetched ${solPrices.length} days of REAL SOL price data`);

    // 3. Scan each candidate token (limit to 6 to avoid rate limits)
    const tokensToScan = candidates.slice(0, 6);
    console.log(`[SpotScanner] Scanning ${tokensToScan.length} candidates: ${tokensToScan.map(t => t.symbol).join(', ')}`);
    ...
    for (let i = 0; i < tokensToScan.length; i++) {
      ...
      try {
        ...
        console.log(`[SpotScanner] [${i + 1}/${tokensToScan.length}] Fetching REAL data for ${tokenInfo.symbol}${pumpfunLabel}...`);
        ...
        if (!currentData) {
          console.log(`[SpotScanner] ❌ ${tokenInfo.symbol} - no current data`);
        ...
        if (!historicalData) {
          ...
          console.log(`[SpotScanner] ⚠️ ${tokenInfo.symbol} - no CoinGecko data, using synthetic history`);
        ...
        try {
          ...
          console.log(`[SpotScanner] ⚠️ ${tokenInfo.symbol} - using default on-chain estimates`);
        }
        ...
        console.log(`\n┌─────────────────────────────────────────────────────────┐`);
        console.log(`│  📊 TOKEN ANALYSIS: ${tokenInfo.symbol.padEnd(38)} │`);
        if (tokenInfo.isPumpFun) {
          console.log(`│  🚀 SOURCE: Pump.fun (AGGRESSIVE MODE)                  │`);
        }
        console.log(`├─────────────────────────────────────────────────────────┤`);
        console.log(`│  Address:         ${tokenInfo.address.substring(0, 20)}...     │`);
        console.log(`│  Price:           $${currentData.price.toFixed(6).padEnd(38)} │`);
        console.log(`│  Market Cap:      $${(currentData.marketCap / 1e6).toFixed(2)}M`.padEnd(60) + '│');
        console.log(`│  Liquidity:       $${(currentData.liquidity / 1e6).toFixed(2)}M`.padEnd(60) + '│');
        console.log(`│  Volume 24h:      $${(currentData.volume24h / 1e6).toFixed(2)}M`.padEnd(60) + '│');
        console.log(`├─────────────────────────────────────────────────────────┤`);
        console.log(`│  Historical Data: ${historicalData.prices.length} days (${historySource})`.padEnd(60) + '│');
        console.log(`│  Holders:         ${holderDisplay}`.padEnd(60) + '│');
        console.log(`│  Token Age:       ${tokenAge} days`.padEnd(60) + '│');
        console.log(`│  Top Holder:      ${topHolderDisplay}`.padEnd(60) + '│');
        console.log(`├─────────────────────────────────────────────────────────┤`);
        console.log(`│  Data Sources:                                          │`);
        console.log(`│  - DexScreener:   ✅ Current price/volume/liquidity     │`);
        console.log(`│  - History:       ${historySource === 'CoinGecko' ? '✅ CoinGecko OHLCV' : '⚠️ Synthetic (API unavailable)'}`.padEnd(60) + '│');
        console.log(`│  - On-chain:      ${holderCount === 1000 ? '⚠️ Default estimates' : '✅ Helius RPC'}`.padEnd(60) + '│');
        if (tokenInfo.isPumpFun) {
          console.log(`│  - Pump.fun API:  ✅ Memecoin metadata                  │`);
        }
        console.log(`└─────────────────────────────────────────────────────────┘`);
        ...
        console.log(`\n❌ [SpotScanner] Failed to scan ${tokenInfo.symbol}`);
    ...
    console.log(`[SpotScanner] ✅ Successfully scanned ${validTokens.length}/${tokensToScan.length} tokens with REAL data`);

    return validTokens;
  } catch (error) {
    console.error('[SpotScanner] ❌ Error fetching spot tokens:', error);
    return [];
  }
}
