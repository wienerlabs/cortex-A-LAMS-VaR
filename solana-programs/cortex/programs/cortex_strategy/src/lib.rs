...
pub mod cortex_strategy {
    ...
    pub fn initialize(ctx: Context<Initialize>, name: String, protocol: String) -> Result<()> {
        require!(name.len() <= 32, StrategyError::NameTooLong);
        require!(protocol.len() <= 32, StrategyError::NameTooLong);
    ...
    pub fn withdraw(ctx: Context<StrategyWithdraw>, amount: u64) -> Result<()> {
        ...
        let strategy = &mut ctx.accounts.strategy;
        let seeds = &[b"strategy".as_ref(), strategy.vault.as_ref(), strategy.name.as_bytes(), &[strategy.bump]];
        let signer = &[&seeds[..]];
...
pub struct Initialize<'info> {
    #[account(
        ...
        seeds = [b"strategy", vault.key().as_ref(), name.as_bytes()], bump
    )]
...
pub struct StrategyDeposit<'info> {
    #[account(mut, seeds = [b"strategy", vault.key().as_ref(), strategy.name.as_bytes()], bump = strategy.bump)]
    pub strategy: Account<'info, Strategy>,
...
pub struct Harvest<'info> {
    #[account(mut, seeds = [b"strategy", strategy.vault.as_ref(), strategy.name.as_bytes()], bump = strategy.bump)]
    pub strategy: Account<'info, Strategy>,
...
pub struct SetActive<'info> {
    #[account(mut, seeds = [b"strategy", strategy.vault.as_ref(), strategy.name.as_bytes()], bump = strategy.bump, has_one = authority)]
    pub strategy: Account<'info, Strategy>,
    pub authority: Signer<'info>,
}
